...
static int addNode(LinkedList* this, int nodeIndex,void* pElement) {
	...
	if (this != NULL && auxNode != NULL && nodeIndex >= 0	&& nodeIndex <= ll_len(this)) {
		...
		if (nodeIndex == 0) {
			...
			auxNodePrevious = getNode(this, nodeIndex - 1);
			if (auxNodePrevious != NULL) {
				auxNode->pNextNode = auxNodePrevious->pNextNode;
				auxNodePrevious->pNextNode = auxNode;
				this->size++;
				returnAux = 0;
			}
		}
...
int ll_remove(LinkedList *this, int index) {
	int returnAux = -1;
	Node *auxToRemove = NULL;
	Node *auxPrevious = NULL;

	if (this != NULL && index >= 0 && index < ll_len(this)) {
		auxToRemove = getNode(this, index);
		if (index == 0 && auxToRemove != NULL) {
			this->pFirstNode = auxToRemove->pNextNode;
			this->size--;
			returnAux = 0;
		} else {
			auxPrevious = getNode(this, index - 1);
			if (auxPrevious != NULL) {
				auxPrevious->pNextNode = auxToRemove->pNextNode;
				this->size--;
				returnAux = 0;
			}
		}
...
int ll_indexOf(LinkedList *this, void *pElement) {
	...
	if (this != NULL) {
		for (int i = 0; i < ll_len(this); i++) {
			index = getNode(this, i);
			if (index != NULL && index->pElement == pElement) {
				returnAux = i;
				break;
			}
		}
